...
class SentenceClusteringLayer(nn.Module):
    ...
    Returns:
      cluster_embedding : (batch_size, d_model)
    """
...
class BGEM3ClusterModel(nn.Module):
    ...
    def load_model(self, model_name, colbert_dim: int = -1):
        ...
        self.clustering_layer = SentenceClusteringLayer(
            dff=self.model.config.intermediate_size,
        ...
        #
        if os.path.exists(os.path.join(model_name, "clustering_layer.pt")):
            logger.info("loading existing clustering_layer---------")
            self.load_clustering_pooler(model_dir=model_name)
        else:
            logger.info(
    ...
    #
    def cluster_embedding(self, last_hidden_state, mask):
        dense_embedding = self.dense_embedding(last_hidden_state, mask)
        cluster_embedding = self.clustering_layer(dense_embedding)
    ...
    #
    def cluster_score(self, q_reps, p_reps):
        scores = self.compute_similarity(q_reps, p_reps) / self.temperature
        scores = scores.view(q_reps.size(0), -1)
        return scores

    def _encode(self, features):
        dense_vecs, sparse_vecs, colbert_vecs, cluster_vecs = None, None, None, None
        ...
        if self.unified_finetuning:
            ...
            #
            cluster_vecs = self.cluster_embedding(
                last_hidden_state, features["attention_mask"]
            )
        if self.normlized:
        ...
        return dense_vecs, sparse_vecs, colbert_vecs, cluster_vecs
    ...
    def encode(self, features, sub_batch_size=None):
        ...
        if sub_batch_size is not None and sub_batch_size != -1:
            all_dense_vecs, all_sparse_vecs, all_colbert_vecs, all_cluster_vecs = (
                [],
                [],
                [],
                [],
            ...
            for i in range(0, len(features["attention_mask"]), sub_batch_size):
                ...
                dense_vecs, sparse_vecs, colbert_vecs, cluster_vecs = self._encode(
                    sub_features
                )
                ...
                #
                all_cluster_vecs.append(cluster_vecs)
            ...
            if self.unified_finetuning:
                ...
                #
                cluster_vecs = torch.cat(all_cluster_vecs, 0)
        else:
            dense_vecs, sparse_vecs, colbert_vecs, cluster_vecs = self._encode(features)
    ...
    def forward(
        ...
        if self.enable_sub_batch:
            q_dense_vecs, q_sparse_vecs, q_colbert_vecs, q_cluster_vecs = self.encode(
                query, sub_batch_size=self.compute_sub_batch_size(query)
            )
            p_dense_vecs, p_sparse_vecs, p_colbert_vecs, p_cluster_vecs = self.encode(
        ...
        if self.training:
            if teacher_scores is not None:
                ...
                if self.unified_finetuning:
                    ...
                    # cluster loss
                    cluster_scores = self.cluster_score(
                        q_cluster_vecs, p_cluster_vecs, q_mask=query["attention_mask"]
                    )  # B, B * N
                    cluster_loss = self.distill_loss(
                        teacher_targets, cluster_scores, group_size=group_size
                    )
            ...
            if (
                ...
                #
                ensemble_distill_cluster_loss = -torch.mean(
                    torch.sum(
                        torch.log_softmax(cluster_scores, dim=-1) * teacher_targets,
                        dim=-1,
                    )
                )
                #
                ...
                loss += (
                    ...
                    + 0.1 * ensemble_distill_sparse_loss
                    + ensemble_distill_colbert_loss
                    + 0.1 * ensemble_distill_cluster_loss
                ) / 4
    ...
    def save(self, output_dir: str):
        ...
        if self.unified_finetuning:
            ...
            #
            torch.save(
                _trans_state_dict(self.clustering_layer.state_dict()),
                os.path.join(output_dir, "clustering_layer.pt"),
            )
            #
    ...
    #
    def load_clustering_pooler(self, model_dir):
        cluster_state_dict = torch.load(
            os.path.join(model_dir, "clustering_layer.pt"), map_location="cpu"
        )
        self.clustering_layer.load_state_dict(cluster_state_dict)


class BGEM3ClusterForInference(BGEM3ClusterModel):
    ...
    def forward(
        ...
        assert (
            return_dense or return_sparse or return_colbert or return_cluster
        ), "Must choose one or more from `return_colbert`, `return_sparse`, `return_dense`, return_cluster to set `True`!"
        ...
        if return_cluster:
            ...
            output["cluster_vecs"] = cluster_vecs
        ...
        if self.normlized:
            ...
            #
            if "cluster_vecs" in output:
                output["cluster_vecs"] = torch.nn.functional.normalize(
                    output["cluster_vecs"], dim=-1
                )
...
path = "/data2/local_datasets/encoder/bgem3_custom/"
