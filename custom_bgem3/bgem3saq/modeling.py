...
# Selft-Attention over Query Embeddings
class BGEM3SaqModel(AbsEmbedderModel):  # class AbsEmbedderModel(ABC, nn.Module):
    ...
    def _colbert_embedding(self, last_hidden_state, mask):
        ...
        colbert_vecs = self.colbert_linear(
            last_hidden_state[:, 1:]
        )  # (batch_size, seq_len-1, hidden_size) @ (hidden_size, colbert_dim) -> (batch_size, seq_len-1, colbert_dim)
    ...
    def _sentence_attention_embedding(self, last_hidden_state, mask):
        ...
        return sentence_attention_embedding # (batch_size, hidden_size)
    ...
    def forward(
        ...
        if self.training or self.model_eval:
            ...
            if self.unified_finetuning:
                ...
                # Attention only over query embeddings
            ...
            logger.warning("model.model_eval : %s",self.model_eval)
            logger.warning("model.training : %s", self.training)
            loss = None
    ...
    def save(self, output_dir: str):
        ...
        if self.unified_finetuning:
            torch.save(
                _trans_state_dict_append_prefix(self.colbert_linear.state_dict(), "colbert_linear"), # colbert_linear
                os.path.join(output_dir, "colbert_linear.pt"),
            )
            torch.save(
                _trans_state_dict_append_prefix(self.sparse_linear.state_dict(), "sparse_linear"), # sparse_linear
...
class BGEM3SaqModelForInference(BGEM3SaqModel):
    """
